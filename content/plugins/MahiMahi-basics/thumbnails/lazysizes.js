(function(window, factory) {
	...
	var loader = (function() {
		...
		var checkElements = function() {
			...
			if (eLlen) {
				...
				for (; i < eLlen; i++, checkElementsIndex++) {
					...
					if (isLoading > 6 && (!elemExpandVal || ('src' in lazyloadElems[i]))) {
						continue;
					}
		...
		var unveilElement = function(elem, force) {
			...
			if ((isAuto || (!supportNativeLQIP && !isCompleted)) && isImg && curSrc && !elem.complete && !hasClass(elem, lazySizesConfig.errorClass)) {
				return;
			}
